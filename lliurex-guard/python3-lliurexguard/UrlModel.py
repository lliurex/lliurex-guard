...
class UrlModel(QtCore.QAbstractListModel):
	...
	UrlIdRole=QtCore.Qt.UserRole+1000
	...
	def data(self, index, role=QtCore.Qt.DisplayRole):
		...
		if 0 <= index.row() < self.rowCount() and index.isValid():
			...
			if role==UrlModel.UrlIdRole:
				return item["urlId"]
			elif role == UrlModel.UrlRole:
				return item["url"]
	...
	def roleNames(self):
		...
		roles = dict()
		roles[UrlModel.UrlIdRole]=b"urlId"
	...
	def appendRow(self,ui,u):
		...
		if ui not in tmpId:
			...
			self._entries.append(dict(urlId=ui,url=u))
			self.endInsertRows()
